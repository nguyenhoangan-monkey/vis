...
def parse_HPC(group_name: str, search_config):
    ...
    print("PARSING HPC DATA...")
    command = (
        f"ls -lt ../*-*-*csv "
        f"| awk '{{print $9}}' "
        f"| sed -n '/{search_config['endDate'].date()}/,/{search_config['startDate'].date()}/p' "
        "| tac"
    ) # command is fragile, should replace with native Python functions
