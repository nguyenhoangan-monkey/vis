...
if args.group[0] == "Com Center Main Room":
    headerData = "Total"
...
args.group = args.group.split(',') # split them into an array with each PDU name
            ...
            with open(file, 'r') as f:
                ...
                for row in reader:
                    if (datetime.timestamp(startDate) <= int(row['Date']) and datetime.timestamp(endDate) >= int(row['Date'])): # timestamp in range
                        ...
                        if len(args.group) > 1:
                            for pdu in args.group:
                                if pdu in hpc_data:
                                    hpc_data[pdu].append(float(row[pdu]))
                                else: 
                                    hpc_data[pdu] = [float(row[pdu])]
                        elif args.group[0] not in row:
                            hpc_data[args.group[0]].append(float(0))
                        elif (args.group[0] == 'Com Center Main Room'): #FOR COMPUTING CENTER MAIN ROOM CAlCUlATIONS, RECORD
                            ...
                            hpc_data[args.group[0]].append(float(row['SeaWulf Main Room on UPS']) + float(row['SeaWulf Main Room on Non-UPS']))
                            ...
                            hpc_data[args.group[0]].append(float(row[args.group[0]]))
        ...
        for file in files:
            with open(file, 'r') as f:
                ...
                for row in reader:
                    ...
                    ent_data[args.group[0]].append(power)
...
if (len(hpc_data[args.group[0]]) < int(args.numPoints)):
    print("Cannot have more points than there are data")
    exit()
interval = int(len(hpc_data[args.group[0]]) / int(args.numPoints)) # determines data point per entry
print("HPC LENGTH:", len(hpc_data[args.group[0]]))

if args.group[0] == 'Com Center Main Room' and not hpcOnly and not upsOnly: # INCLUDE ENTERPRISE EQUIPMENT DATA
    parse_ENT()
    for key in ent_data:
        print(key, ent_data[key][:10])
    print("ENT LENGTH:", len(ent_data[args.group[0]]))

if args.group[0] == 'Com Center Main Room' and not hpcOnly and not entOnly: 
    parse_UPS()
    for key in ups_data:
...
print("\n\n\n")
print(ups_data['Date'][0])
print("\n\n")
print(hpc_data['Date'][0])
...
AUX_GROUP_NAME = args.group[0]
if args.group[0] == 'Com Center Main Room': # MAIN ROOM CALCULATIONS
...
elif args.group[0] == 'Com Center Annex Total':
    for x in range(0, int(args.numPoints)): 
        if args.avg: 
            if (hpc_data['Date'][x*interval] >= 1710302406): # if data's past March 13th
                annex_load = (sum(hpc_data[args.group[0]][x*interval : (x+1)*interval]) / interval) + ANNEX_NONUPS + SCGP_LOAD
            elif (hpc_data['Date'][x*interval] >= 1708059906): # if data's past February 16th
                annex_load = (sum(hpc_data[args.group[0]][x*interval : (x+1)*interval]) / interval) + ANNEX_A03 + ANNEX_NONUPS + SCGP_LOAD
            else:
        ...
        if args.max: # for the -m flag and default behaviosr
            # print("MAX:" , max(hpc_data[args.group[0]][x*interval : (x+1)*interval]))
            if(hpc_data['Date'][x*interval] >= 1710302406):  # if data's past March 13th
                annex_load = max(hpc_data[args.group[0]][x*interval : (x+1)*interval]) + SCGP_LOAD
            elif(hpc_data['Date'][x*interval] >= 1708059906):  # if data's past February 16th
                annex_load = max(hpc_data[args.group[0]][x*interval : (x+1)*interval]) + SCGP_LOAD + ANNEX_A03
            else: 
                annex_load = ANNEX_UPS
...
elif args.group[0] == 'SeaWulf Annex on UPS':
    for x in range(0, int(args.numPoints)): 
        if args.avg: 
            if (hpc_data['Date'][x*interval] >= 1710302406): # if data's past March 13th
                annex_load = (sum(hpc_data[args.group[0]][x*interval : (x+1)*interval]) / interval)
            elif (hpc_data['Date'][x*interval] >= 1708059906):
                annex_load = (sum(hpc_data[args.group[0]][x*interval : (x+1)*interval]) / interval) + ANNEX_A03
            else:
                annex_load = ANNEX_UPS
            datetime_obj = datetime.fromtimestamp(round(sum(hpc_data['Date'][x*interval : (x+1)*interval]) / interval))
            date = datetime_obj.strftime("%m/%d-%H:%M")
        ...
        if args.max: # for the -m flag and default behaviosr
            if (hpc_data['Date'][x*interval] >= 1710302406): # if data's past March 13th
                annex_load = (max(hpc_data[args.group[0]][x*interval : (x+1)*interval]))
            elif (hpc_data['Date'][x*interval] >= 1708059906):
                annex_load = (max(hpc_data[args.group[0]][x*interval : (x+1)*interval])) + ANNEX_A03
            else: 
                annex_load = ANNEX_UPS
            # print("MAX CALCULATED:" , annex_load)
            datetime_obj = datetime.fromtimestamp(round(sum(hpc_data['Date'][x*interval : (x+1)*interval]) / interval))
    ...
    if len(args.group) > 1:
        for pdu in args.group:
            if len(hpc_data['combined']) == 0:
                hpc_data['combined'] = np.array(hpc_data[pdu])
            else:
                hpc_data['combined'] += np.array(hpc_data[pdu])
        AUX_GROUP_NAME = ', '.join(args.group)
        args.group = ['combined']
    for x in range(0, int(args.numPoints)):
        if args.avg: # for the -a flag and default behavior
            average = round(sum(hpc_data[args.group[0]][x*interval : (x+1)*interval]) / interval, 2)
            datetime_obj = datetime.fromtimestamp(round(sum(hpc_data['Date'][x*interval : (x+1)*interval]) / interval))
            date = datetime_obj.strftime("%m/%d-%H:%M")
            averages[date]  = average
        if args.max: # for the -m flag and default behavior
            maxim = round(max(hpc_data[args.group[0]][x*interval : (x+1)*interval]), 2)
            datetime_obj = datetime.fromtimestamp(round(sum(hpc_data['Date'][x*interval : (x+1)*interval]) / interval))
            date = datetime_obj.strftime("%m/%d-%H:%M")
            maxes[date]  = maxim
...
ax.set_title(f'Power Data for {AUX_GROUP_NAME} {headerData}', y=1.07)
ax.set_xlabel('Time')
ax.set_ylabel('Power usage (kW)')
# ax.set_ylim(min(filedata[args.group[0]]) - totMax * 0.05, totMax + totMax * 0.1)
...
if(numDays < 5 and int(args.numPoints) < 26): #labels 13 ticks
    ...
    if(numDays > 21):
        interval = (numDays // 80) + (numDays % 30 > 0)
    else:
        interval = 1
    x = 1
    for i in range(1,len(ticks)):
        print(ticks[i].get_text())
        if(ticks[i].get_text()[0:5] == ticks[i-1].get_text()[0:5]): # extracts only the date from the text
             ticks[i].set_visible(False)
        else:
             if(x % interval != 0):
                 ticks[i].set_visible(False)
